...
하루 권장량을 기본 20% 이상 채우는 경우만 사용합니다. DB 원재료에 안전한 활용 요리
매핑이 2개 미만이면 OpenAI가 해당 원재료의 요리만 보완하며, DB 후보 자체가 부족할
때에는 OpenAI가 원재료 후보까지 생성합니다. 두 경로 모두 알레르기·비선호 검증을 다시
적용합니다. 새 추천 생성은 사용자의 직전 추천 원재료를 제외하고, 추천 새로고침은 해당
추천에서 이전에 노출된 모든 원재료 이름을 누적 제외합니다.
