...
`RecommendationPlanningService`는 활성 매핑이 2개 이상 존재하는 원재료 ID를 먼저 조회하고 해당 Food만 영양 랭킹 대상으로 사용한다. 음식 영양은 100개, 요리 매핑은 20개 원재료 단위로 조회하며 안전한 원재료 3개를 확보하면 남은 매핑 배치를 조회하지 않는다.
...
seed는 같은 `ingredient_name`에 추천 대표 Food가 여러 건 존재하는 비정상 데이터에서도 `food_id`가 가장 작은 한 건을 결정적으로 선택한다. 재적재로 대표 Food가 바뀌면 기존 매핑의 `ingredient_food_id`도 함께 갱신한다.
